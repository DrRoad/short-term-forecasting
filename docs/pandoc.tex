...
% modified by nmstreethran@gmail.com 
%%%
...
$if(fontfamily)$
  ...
  \allsectionsfont{\normalfont\sffamily\bfseries} % ^ bold + sans serif section headings
  %%%
$else$
  \usepackage{lmodern}
$endif$
...
$endif$
\usepackage{listings} 
% removed listings from 'if' macro as it causes '! LaTeX Error: Environment lstlisting undefined.' when compiling PDF
\lstset{
  breaklines=true, % add automatic line breaks
...
$if(title)$
  $if(beamer)$
    \frame{\titlepage}
  $else$
    \IfFileExists{titlepage.tex}{\input{titlepage.tex}}{\maketitle}
  $endif$
